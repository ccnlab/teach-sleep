	...
	{Name: "Base", Desc: "these are the best params", Sheets: params.Sheets{
		"Network": &params.Sheet{
			...
			{Sel: "#CA3ToCA3", Desc: "CA3 recurrent cons",
				Params: params.Params{
					"Prjn.WtScale.Rel": "2", // 2 > 1 > .5 = .1  used to be .1
			...
			{Sel: "#ECinToDG", Desc: "DG learning is surprisingly critical: maxed out fast, hebbian works best",
				Params: params.Params{
					...
					"Prjn.CHL.Hebb":    ".5",   // check this out .5 > 1 overall March 28 tried this
					...
					"Prjn.Learn.Lrate":       "0.4", // .4 > .3 > .2 March 28 tried this
					"Prjn.Learn.Momentum.On": "false",
					"Prjn.Learn.Norm.On":     "false",
					"Prjn.Learn.WtBal.On":    "true",
				}},
...
type Sim struct {
	...
	Roundpertrial      int                         `desc:"how many repetitions per training trial"`
	PreTrainEpcs       int                         `desc:"number of epochs to run for pretraining"`
	HipTrainEpcs       int                         `desc:"number of epochs to run for hip train"`
	SemTrainEpcs       int                         `desc:"number of epochs to run for sem train"`
	RPTrainEpcs        int                         `desc:"number of epochs to run for RP train"`
	SleepTrainEpcs     int                         `desc:"number of epochs to run for sleep train"`
	BaseSleepTrainEpcs int                         `desc:"number of epochs to run for sleep train"`
...
func (ss *Sim) ConfigEnv() {
	...
	if ss.MaxEpcs == 0 { // allow user override
		ss.MaxEpcs = 1
		ss.PreTrainEpcs = 0 // seems sufficient? increase?
		ss.Roundpertrial = 1
	}
	...
	for qtr := 0; qtr < 4; qtr++ {
		...
		case 1: // Second, Third Quarters: CA1 is driven by CA3 recall
			...
			if train {
				...
				ca3FmDg.WtScale.Rel = dgwtscale - 3 // testing
			}
...
func (ss *Sim) AlphaCycSleep(train bool) {
	...
	ca1FmCa3.WtScale.Abs = 0
	dgwtscale := ca3FmDg.WtScale.Rel
	ca3FmDg.WtScale.Rel = dgwtscale - 4 // turn off DG input to CA3 in first quarter
	ss.Net.GScaleFmAvgAct()             // update computed scaling factors
	ss.Net.InitGInc()                   // scaling params change, so need to recompute all netins
...
func (ss *Sim) SaveTstTrial(Subject, Condition, Phase string) {
	var err error
	HipTrainEpcs := strconv.Itoa(ss.HipTrainEpcs)
	RPTrainEpcs := strconv.Itoa(ss.RPTrainEpcs)
	SleepTrainEpcs := strconv.Itoa(ss.SleepTrainEpcs)
	BaseSleepTrainEpcs := strconv.Itoa(ss.BaseSleepTrainEpcs)
	fnm := ss.Tag + "_" + ss.Net.Nm + "_" + "Hip" + HipTrainEpcs + "_" + "RP" + RPTrainEpcs + "_" + "Sleep" + SleepTrainEpcs + "_" + "BaseSleep" + BaseSleepTrainEpcs + "_" + "Sub" + Subject + "_" + Condition + "_" + Phase + ".txt"
...
func (ss *Sim) CloseUnrelated() { //30 7 20 1 19 1 works in this condition
	...
	for sub := 0 + ss.Group*ss.MaxSubs; sub < ss.MaxSubs+ss.Group*ss.MaxSubs; sub++ {
		...
		ss.SetEnv(false)
		if ss.BaseSleepTrainEpcs != 0 {
			ss.MaxEpcs = ss.BaseSleepTrainEpcs
			ss.TrainSleep()
		}
		ss.OpenPatsSleep()
		ss.SetEnv(false)
		ss.MaxEpcs = ss.SleepTrainEpcs
		ss.TrainSleep()
		...
		ss.OpenPatsRP()
		ss.MaxEpcs = ss.RPTrainEpcs * 2
		ss.SetEnv(false)
		ss.TrainRetrievalPractice()
		ss.SetEnv(false)
		ss.SaveTstTrial(Subject, Condition, "221rp+")
		ss.OpenPatsSleepFull()
		ss.SaveTstTrial(Subject, Condition, "222rp+")
		...
		ss.OpenPatsSleepWeak()
		ss.SetEnv(false)
		ss.MaxEpcs = ss.SleepTrainEpcs
		ss.TrainSleep()
		ss.SaveTstTrial(Subject, Condition, "332weak")
		...
		ss.MaxEpcs = ss.SleepTrainEpcs
		ss.TrainSleep()
		ss.SaveTstTrial(Subject, Condition, "332full")
		...
		ss.SetEnv(false)
		ss.MaxEpcs = ss.SleepTrainEpcs - ss.BaseSleepTrainEpcs
		ss.TrainSleep()
		ss.SaveTstTrial(Subject, Condition, "112weak")
	}
}
...
func (ss *Sim) OpenPatsSleepWeak() {
	...
	// patgen.ReshapeCppFile(ss.TestFar, "Test_AC.dat", "TestFar.dat")
	// patgen.ReshapeCppFile(ss.TestLure, "Lure.dat", "TestLure.dat")
	ss.OpenPat(ss.TrainClose, "sleep_weak.txt", "Close Training Patterns")
	ss.OpenPat(ss.TrainFar, "sleep_weak.txt", "Far Training Patterns")
	ss.OpenPat(ss.TestClose, "final_test.txt", "Close Testing Patterns")
	ss.OpenPat(ss.TestFar, "final_test.txt", "Far Testing Patterns")
}
...
func (ss *Sim) ConfigGui() *gi.Window {
	...
	trmen.Menu.AddAction(gi.ActOpts{Label: "PreTrain", Icon: "run", Tooltip: "Starts the network training, picking up from wherever it may have left off.  If not stopped, training will complete the specified number of Runs through the full number of Epochs of training, with testing automatically occuring at the specified interval.",
...
func (ss *Sim) CmdArgs() {
	...
	flag.IntVar(&ss.HipTrainEpcs, "hipepc", 8, "hip train epochs")
	flag.IntVar(&ss.RPTrainEpcs, "rpepc", 16, "rp train epochs")
	flag.IntVar(&ss.SleepTrainEpcs, "sleepepc", 6, "hip train epochs")
	flag.IntVar(&ss.BaseSleepTrainEpcs, "basesleepepc", 2, "hip train epochs")
	...
	flag.BoolVar(&ss.Daydream, "daydream", false, "whether to have day dream")
	flag.Parse()
	fmt.Printf("tag:" + ss.Tag + "\n")
	fmt.Printf("Running %d Subs %d Group %d sem %d hipepc %d rpepc %d sleepepc %d basesleepepc %v outec\n", ss.MaxSubs, ss.Group, ss.SemWake, ss.HipTrainEpcs, ss.RPTrainEpcs, ss.SleepTrainEpcs, ss.BaseSleepTrainEpcs, ss.Outecback)
	if ss.Daydream {
